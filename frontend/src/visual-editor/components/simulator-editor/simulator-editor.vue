<template>
  <div class="simulator-container" @mousedown="deselectComp()">
    <div class="simulator-editor">
      <div :class="[appData.id]" class="simulator-editor-content">
...
<script lang="tsx" setup>
  ...
  const { currentPage, setCurrentBlock, currentBlock, jsonData: appData } = useVisualData();
  ...
  watchEffect(() => {
    ...
    const { css } = currentPage.value.config;
    ...
    style.innerHTML = TemplateParser.parser.parse(
      `${css ?? ''}`,
      /*@ts-ignore*/
      appData,
      currentPage.value,

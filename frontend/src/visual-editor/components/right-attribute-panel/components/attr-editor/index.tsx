    ...
    const FormEditor = () => {
      ...
      return (
        <>
          ...
          <ElCarousel indicator indicatorPosition={'outside'} height={'82px'} arrow={'never'}>
            {[
              {
                trick: '当前未选择组件，您也可以前往配置页面',
              },
              {
                trick: '项目采用的是 mustache 模板语法',
              },
            ].map((item) => (
              <ElCarouselItem key={item.trick}>
                <Alert class={'cursor-pointer'}>
                  <AlertTitle>注意！</AlertTitle>
                  <AlertDescription>{item.trick}</AlertDescription>
                </Alert>
              </ElCarouselItem>
            ))}
          </ElCarousel>
